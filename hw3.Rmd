...
pacman::p_load(tidyverse,lubridate,lmtest)
```
...
reject5pct <- sum(pvals < 0.10)
print(paste0('Number of Rejections at 5% Significance Level: ', reject5pct))
print(paste0('Power: ', reject5pct / 1000 * 100, '%'))
```
...
for (i in 1:1000) {
  df <- dgp_cluster()
  model <- lm(plot_yield ~ treat + village, data = df)
  pval <- summary(model)$coefficients['treat','Pr(>|t|)']
  pvals <- c(pvals,pval)
}
